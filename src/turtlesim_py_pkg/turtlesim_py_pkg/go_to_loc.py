...
class gotolocationNode(Node):
    def __init__(self):
        ...
        self.coeff = 1.5
        self.pose_thresholed_linear= 0.2
        self.pose_thresholed_angular = 0.01
        self.target_x = 4.0 #0.0-11.0 [info: defult loc x -5.5]
        self.target_y = 9.0 #0.0-11.0 [info: defult loc x -5.5]
        self.pose_ = None
    ...
    def turtle_controller(self):
        ...
        if abs(target_theta - self.pose_.theta) > self.pose_thresholed_angular:
            msg.angular.z = (target_theta - self.pose_.theta) * self.coeff

        else:
            if distance >= self.pose_thresholed_linear:
    ...
    def callback_call_catch_turtle(self, future, turtle_name):
             try:
                 ...
                 self.get_logger().info("Turtle" + response.name + "could not be caught!")
